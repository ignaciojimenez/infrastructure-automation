...
- **unifi-lxc SSH Access** `V:Low E:Low` — Surfaced during 2026-05-11 SSH-play idempotency rollout: container rejects `choco`'s key with `Permission denied (publickey,gssapi-keyex,gssapi-with-mic)`, so Ansible can't reach it. Pre-existing issue. Either deploy the primary user's public key into the LXC's `authorized_keys`, or accept the LXC as Proxmox-console-only and document.
