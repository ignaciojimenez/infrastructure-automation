...
- **`detect_audio.log` empty after ~10h of active service** `V:Low E:VLow` — Surfaced 2026-07-12 during the vinylstreamer reboot post-mortem: the log read empty when it shouldn't have. Minor anomaly worth a glance next time vinylstreamer is touched. (The other half of that finding — volatile journal — was fixed 2026-07-14: persistent journald with SD-wear caps on all four Pis via the raspberrypi platform playbook, `enable_persistent_journal` toggle; verified across a controlled vinylstreamer reboot. pstore/ramoops considered and skipped: reserved-RAM cost on 512MB Pis and it can't capture power cuts, the most plausible cause of the 07-12 event.)
- **cwwk journal is persistent but uncapped — 2.3G and growing** `V:Low E:VLow` — Surfaced 2026-07-14 while rolling out capped persistent journald to the Pis: cwwk already persists its journal (that's how the THERMTRIP forensics worked) but has no `SystemMaxUse` cap and sits at 2.3G. Not urgent on a big disk, but a one-line drop-in in `platform/proxmox` would bound it. unifi-lxc is persistent at ~100M; opnsense is FreeBSD syslog (persistent by default).
