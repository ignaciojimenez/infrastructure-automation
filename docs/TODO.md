...
literal cron args, visible to `crontab -l` and `ps`. Move to an env file sourced
by `enhanced_monitoring_wrapper`. Needs a coordinated fleet redeploy.
...
## ⚠️ Below here is NOT yet triaged

Everything past this line predates the 2026-08-17 cleanup and is a mix of
resolved, superseded and still-open. **It has not been reviewed item by item.**
Treat a claim here as dated evidence, not current state — several sections are
known to be stale (e.g. `read_agent` on opnsense, which L-H made moot).

Next cleanup pass: work down these sections, promote anything genuinely open into
*What to work on next* above, and distil the rest into `archive/DONE.md`.
