...
- **✅ Fleet pre-flight for L3, run from a phone 2026-08-06 — two findings, one fixed** `V:High E:Low` — Before deploying checks that have never been able to fail, the same probes were run by hand on `cwwk`, `dockassist` and `agent-lxc`. This is what that dry run was for and it earned its keep twice.
  ...
  🔴 **`agent-lxc` has 1 failed unit: `logrotate.service`**, failing since 2026-08-06 00:24 with `status=1/FAILURE`. `check_failed_units` is new, so **this pages agent-lxc the moment L3 lands** — the check working exactly as intended, on the host with the worst history. Root cause not yet established: `read_agent` cannot read the journal (not in `adm`), and a `logrotate -d` dry run as `read_agent` only reproduces its own permission errors. Prime suspect is `/etc/logrotate.d/choco`, which globs `/home/choco/.logs/*.log` — `missingok` covers a missing *file* but not a glob failure on a missing or untraversable directory, and that host's entire 12-day outage was a missing `logs_dir`. **Needs one command as `choco`:** `sudo journalctl -u logrotate.service -n 20 --no-pager`.
