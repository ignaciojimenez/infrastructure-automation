...
- **`read_agent` on OPNsense belongs in `config.xml`, not `/etc/passwd`** — Decided 2026-07-21 after a firmware upgrade (26.1.9, 2026-06-13) silently deleted the Ansible-created account, leaving agent access to the firewall broken for ~3 months. OPNsense regenerates `/etc/passwd` and `sshd_config` from `config.xml`, discarding anything created out-of-band with `pw`; `choco` survived the same upgrade because it is a config.xml user. Fix is to create the account through OPNsense's own user manager (UI or `/api/auth/user`) so it is durable by construction — chosen over monitoring-and-recreating, which would treat a self-inflicted recurring break as a fact of life. Sudoers rules and the home directory are unaffected (`/usr/local/etc/sudoers.d/` is not regenerated — verified by a file predating the upgrade).
