...
- **Backup I/O capped** (2026-09-06, measured) - The nightly 03:00 `vzdump` of VM 100 reads a full 64 GiB disk and is the largest recurring thermal event on the box. Capped to **100 MiB/s** via `proxmox_vzdump_bwlimit_kib` → `/etc/vzdump.conf`. Uncapped: ~288 MiB/s, 4 min, 78–79 °C mean, **125 s above 80 °C**. Capped: ~100 MiB/s, 11 min, 64 °C mean, **15 s above 80 °C**. The **peak barely moves** (86 → 85 °C) — the mechanism is *time at temperature*, so do not expect this knob to lower a maximum. ⚠️ **Never tune it by throttle-event count**: two runs with identical settings scored 48 and 79, so single-run resolution is ~±30 events and finer tuning is below the instrument's noise floor. Time-above-80 °C is the usable metric. `/etc/vzdump.conf` is the **only** place this may be set — PVE merges it as defaults for keys the job does not define (`PVE::VZDump::read_vzdump_defaults`), so a per-job `bwlimit` from the web GUI silently wins and makes edits to the file no-ops; the role strips it from every job.
- **Backup frequency is not a thermal lever** (2026-09-06, measured) - Backing up less often was proposed on wear grounds and **withdrawn on the arithmetic**. Arrhenius over a full day (Ea swept 0.5–0.9 eV; ranking stable): the backup runs at 1.8–2.8× idle wear rate for 12 minutes, adding **0.6–1.4% to a day**; nightly → weekly would save **0.5–2.1% of a week**. cwwk idles at ~53 °C for 99.2% of its life and that floor dominates total wear entirely. **The lever for long-term wear is the idle temperature — vent sizing — not the backup.** Do not re-propose reduced backup frequency as a thermal fix.
- **ZFS compression off on the backup dataset** (2026-09-06) - `rpool/var-lib-vz` stores vzdump's own zstd archives; measured `compressratio` was exactly **1.00x**, i.e. a pointless pass over ~17 GB nightly. Set via `proxmox_backup_dataset_compression`.
- **Backup retention is managed on the job, and cut to 3 daily + 2 weekly** (2026-09-17, measured) - `/var/lib/vz/dump` held **15 daily copies of VM 100 at ~17.4 GiB each = 261 GiB** on a pool at **73% capacity, 55% fragmentation** — so disk, not CPU or RAM, was what actually constrained this box. ⚠️ Unlike `bwlimit`, retention has **no usable file-level default**: `prune-backups` lives on the backup job (`pvesh get /cluster/backup`) and a job value overrides `/etc/vzdump.conf`, so the role sets it per job via `proxmox_vzdump_keep_daily` / `_keep_weekly` and compares before writing, keeping `changed=0` on a converged host. 📌 **This is a recovery decision wearing a disk decision's clothes** — the USB copy keeps only 2 generations, so the local dailies are the deeper history; `keep-weekly=2` preserves reach to a fault noticed a fortnight later for a fifth of the space. Do not conflate with the withdrawn "back up less often" proposal above: **frequency** was rejected on wear arithmetic, **retention** is about space, and the nightly job still runs.
...
- **Fleet writes and signed merges stay laptop-only; everything else moves off it (2026-09-15)** — Phase C's approval gate is a forwarded Secure-Enclave key plus a biometric tap, and `git ms`/`git ts` are touch-required by design; both need a Mac in hand. So the laptop is where work is *approved*, not where it happens. Authoring, PRs, CI and read-only investigation run in two lanes: **hosted Claude Code cloud sessions** for repos that need only GitHub and Cloudflare, and a **workbench container on cwwk** for anything needing the LAN — with **no fleet access** (no `read_agent` key, no vault, no `pct`). Rejected: Coder (a control plane to secure for what tmux + Remote Control already give), growing agent-lxc into a build box (read-only by construction and holds fleet keys — see [Test Environments](#test-environments-2026-08-18) goal 3), and Claude Code self-hosted environments (Team/Enterprise only). Stages are the milestones of the *Build from anywhere* project in Linear. **This repo is deliberately in the cloud lane (2026-09-19):** the Claude GitHub App is installed here, so a phone session can branch and open PRs. That is authoring, not deploying: nothing reaches a host without an Ansible run from the laptop. The exposure it adds is `main` itself, which has no branch protection, so review cloud PRs before merging. A test session reported its open PR as "merged" and got a one-word typo fix wrong.
...
- **Cost reality** — a healthy fleet costs ≈ the weekly digest only (~$2/mo). Tier 1 and the anomaly/Slack triggers are free when nothing is wrong; each *new* real problem is a one-off ~$0.30–0.55 and is then covered by its host's open incident until it clears. Fix a recurring alert and its cost goes to zero. The one-time backlog burst that cleared three accumulated problems ($1.32) is not the run rate.
- **Slack-alert triage is a trusted script, not an agent MCP** — the hourly sweep can't see per-service wrapper failures or HA alerts (they only reach `#home-alerts`). A Slack MCP would (a) put cost-control dedup in the model's non-deterministic hands and (b) widen the sandboxed agent's reach to slack.com with permission semantics OpenCode doesn't document. Instead `investigate.sh --slack` does the read deterministically, with dedup, a per-run cap and a daily spend cap; the agent stays SSH-only and never sees the read token. MCP is the right tool for Phase C (human-present, interactive), not for unattended cost-capped triage.
- **Tier 2 dedups by (host, subject) incident, shared across both modes — not by message text, finding set, or host alone.** On 2026-09-13 one fault (raspotify on hifipi) was billed **6× ($1.57)** overnight: the Slack watch keyed on message text, and one fault produced three texts (health check, check script, STILL FAILING reminder); anomaly mode keyed on the whole finding set, so a second host joining and leaving re-billed the first twice; neither mode saw the other's work. The subject is *what is failing*, read deterministically from the alert (check script name, named unit/service, `reachability`, else a line signature), so all three hifipi texts are `hifipi.raspotify`. **Host-only keying was built first and rejected:** it also collapsed the six runs, but withheld the plan for any *unrelated* fault on a host that already had one open. A new subject on such a host is investigated, and its prompt lists the host's open incidents so causes can still be linked. An incident stays open until unmentioned for longer than the wrapper's maximum reminder gap (`--alert-repeat-max`), so a backed-off reminder cannot read as a recovery; older than 7 days gets one fresh look. A daily spend cap is the backstop; the digest counts toward it but is never blocked. Replayed against that night: **2 runs, ≤ $0.61**. Trade-off accepted: one fault that two checks describe without a shared unit name costs a second run — bounded by the cap, where a hidden fault would not be.
...
- **Mullvad relay health is an alert on change, and fixed by hand** (2026-09-14) — agent-lxc polls Mullvad's public relay API for the relays OPNsense's WireGuard peers use and alerts `#home-alerts` only when one goes inactive, drops out of the list or recovers (weekly reminders to `#home-logging`; never a failing check). **No OPNsense API privilege for gateway state:** every privilege that exposes it also grants write access (*System: Gateways*) or service control (*VPN: WireGuard: Status*), and healthchecks already watches reachability — do not re-propose it. Peer replacement stays manual ([runbook](NETWORK.md#replacing-a-mullvad-peer-runbook)): an automated swap on the internet single point of failure that picks wrong takes VLAN 40/80 offline, and Mullvad never states whether an inactive relay is in maintenance or retired.
