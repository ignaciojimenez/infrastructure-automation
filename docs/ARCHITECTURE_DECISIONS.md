...
- **Tier 2 dedups by host incident, shared across both modes — not by message text or finding set.** On 2026-09-13 one fault (raspotify on hifipi) was billed **6× ($1.57)** overnight: the Slack watch keyed on message text, and one fault produced three texts (health check, check script, STILL FAILING reminder); anomaly mode keyed on the whole finding set, so a second host joining and leaving re-billed the first twice; neither mode saw the other's work. Now an incident per host stays open until the host goes unmentioned for longer than the wrapper's maximum reminder gap (`--alert-repeat-max`), so a backed-off reminder cannot read as a recovery; an incident older than 7 days gets one fresh look. A daily spend cap is the backstop; the digest counts toward it but is never blocked. Replayed against that night: **2 runs, ≤ $0.61**. Trade-off accepted: a second, unrelated fault on the same host inside an open incident still pages, but gets no plan of its own.
